...
public class attaque_zombie : MonoBehaviour
{
    ...
    IEnumerator dps_zombie()
    {
        while (true)
        {
            ...
            {
                if (Vector3.Distance(GameObject.FindWithTag("player").transform.position, gameObject.transform.position) < 5.5f)
                {
                    ...
                    GameObject.FindWithTag("player").GetComponent<pv_player>().Blood_Effect();
